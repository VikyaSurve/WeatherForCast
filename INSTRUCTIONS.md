...
## Preparing your weatherforcast project

Copy the provided `config` and `catalog` directories including the files they contain into your `weatherforcast` project.

You can now follow the instructions below to deploy your project.

## Working on your weatherforcast project
...
```
tanzu apps workload create weatherforcast -f config/workload.yaml \
  --local-path . \
  --source-image <REPOSITORY-PREFIX>/weatherforcast-source \
  --type web
```
...
```
tanzu apps workload get weatherforcast
```
